...
const itemTitles = [
	"Project Alpha", "Dashboard Component", "API Integration", "Database Migration",
	"User Authentication", "Payment Gateway", "Notification System", "Analytics Module",
	"Search Index", "Cache Layer", "Queue Processor", "Webhook Handler",
	"Report Generator", "Export Service", "Import Pipeline", "Email Service",
	"File Storage", "Image Resizer", "PDF Generator", "Chat System"
];

const itemDescriptions = [
	"High priority feature for Q1", "Performance optimization required", "Integration with third-party API",
	"Migrate legacy data to new schema", "OAuth2 and JWT implementation", "Stripe and PayPal support",
	"Push and email notifications", "Real-time metrics dashboard", "Elasticsearch integration",
	"Redis caching strategy", "Background job processing", "Event-driven architecture",
	"Scheduled report generation", "CSV and Excel export", "Batch import from external sources",
	"Transactional email templates", "S3-compatible storage", "On-the-fly image processing",
	"HTML to PDF conversion", "WebSocket-based messaging"
];
	...
	return html`
		...
		">
			...
			<div class="item-info" style="flex: 1; min-width: 0;">
				<div class="item-title" style="font-weight: 600; color: #333; margin-bottom: 0.25rem;">
					${item.title}
				</div>
				<div class="item-description" style="color: #666; font-size: 0.9rem;">
					${item.description}
				</div>
			</div>
			<div class="item-id" style="color: #999; font-size: 0.85rem;">
				#${item.id}
			</div>
		</div>
	`;
