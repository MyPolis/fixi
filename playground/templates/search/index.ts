...
export function searchItems(query: string, limit: number = 5): SearchResult[] {
...
export function renderSearchResult(item: SearchResult, query: string, index: number): string {
	...
	return html`
		...
		>
			...
			>
				<div
					class="result-title"
					style="font-weight: 600; color: #333; margin-bottom: 0.25rem; font-size: 1rem;"
				>
					${highlightedTitle}
				</div>
				<div
...
export function renderSearchResults(query: string): string {
	...
	const resultsHtml = results.map((item, index) => renderSearchResult(item, query, index)).join("");
...
export function createSearchResponse(query: string): string {
	...
	return html` <div class="search-results-container">${resultsHtml}</div> `;
}
...
export function renderSkeletonLoader(count: number = 5): string {
	...
	return html` <div class="skeleton-container">${skeletonItems}</div> `;
}
